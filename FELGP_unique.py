...
import pca_felgp_functions as felgp_fs
from strongGPDataType import Int1, Int2, Int3, Int4, Int5, Int6, Int7, Int8
...
'''
研究的主要议题是能否让种群中的个体都不一致，即在空间中有效搜索，且保持物种多样性
将PCA放进演化部分
限制子代个体一致
'''
...
# Classification
pset.addPrimitive(felgp_fs.linear_svm, [Array5, Array2, Int4], Array6, name='SVM')
...
# PCA
# 我的计划是放PCA和kpca，输入的类型要相同，输出的类型也要相同，然后在PCA中取得是能量0.95，但是在kpca中取的长度是可变的
pset.addPrimitive(felgp_fs.PCA_ori, [Array4, Int8], Array5, name='PCA')
pset.addPrimitive(felgp_fs.kpca, [Array4, Int7], Array5, name='kPCA')
...
pset.addEphemeralConstant('C', lambda: random.randint(3, 5), Int4)
...
# 加入kpca中取平均长度的范围
pset.addEphemeralConstant('k_lenght', lambda: random.randint(1, 5), Int7)
pset.addEphemeralConstant('lenght', lambda: random.randint(2, 10), Int8)
##
...
def evalTrain(toolbox, individual, hof, trainData, trainLabel):
    ...
    if len(hof) != 0 and individual in hof:
        ...
        try:
            # 在这里出现了问题，导致有些个体的最低适应度值为0,为什么会出现有些个体的适应度值为0的情况？
            # print('func:',individual)
            func = toolbox.compile(expr=individual)
            # print("Is individual compiled?", hasattr(individual, "compiled"))
            ...
            # print('在try中正确运算：',accuracy)
        except Exception as e:
            print("Compilation failed with error:", e)
            accuracy = 0
            # print('accuracy:',accuracy)
    # print('最终的acc:',accuracy)
    return accuracy,
